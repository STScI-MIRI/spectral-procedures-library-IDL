FUNCTION rd_spfits,infile,FITS_HDR=fhdr,SMART=smart,_extra=e

; version 2.3 (+)

; 24 Jun 13 modified to handle CASSIS data
; 22 Oct 12 added _extra - /silent must be explicit in rd_spfits call 
;           also repairing a bug so that old spec FITS files can be read
...
a = readfits(infile,hdr,_extra=e)
ncols = n_elements(a[*,0])
nrows = n_elements(a[0,*])
...
for i=0,ncols-1 do begin
  ...
  case strtrim(labels[i],1) of
    'wavelength': waveflag=1
    'flux'      : fluxflag=1
    'flux_error': fuflag=1
    'error (RMS+SYS)' : fuflag=1
    'order'     : ordflag=1
    'segment'   : ordflag=1
    'flag'      : flagflag=1
    '0'         : undef_col[i] = 1
