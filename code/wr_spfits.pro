...
;  6 Aug 14 2.4 GCS - changed len_array to be long if redefined
;  2 Nov 12 2.4 GCS - correcting ocol to point to col 1 if only 2 cols
; 27 Jan 11 2.4 GCS - no longer adding a 3rd column to 2-column input
;  4 Aug 09 2.3 GCS - modified to avoid crash for only one wavelength element 
; 15 May 09 2.3 GCS - remove the business about checking for '/' in file name
...
ndim=sz_data[0]
ncol=sz_data[1]
if (ndim ge 2) then nrow=sz_data[2] else nrow=1
if (ndim gt 2) then begin
  errflag=1
  print,errtext+'  Spectral data array must have 1-2 dimensions.'
endif
...
;   determine order column (ocol) or assume default (min of 3 or last col)
  ...
  if (ocol eq -1) then begin
    ...
    ocol=min([ncol-1,3]) ; default order column is min of 3 and last col
  endif
...
; 27 Jan 11 - no longer adding a column if ncol eq 2
...
; if errflag not set, then write spectral FITS file, else print file name
