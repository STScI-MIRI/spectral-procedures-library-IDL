FUNCTION spgridres,spold,spnew,fwhm,POLYFWHM=polyfwhm,RANGE=range,$
  VERBOSE=verbose

; 30 Nov 20 ensured loop variables are long
;  7 May 20 changing FWHM to a vector with one value per wavelength element
;           for FWHM as a polynomial f'n of wavelength, use polyfwhm keyword
;  8 Jan 14 debugging
...
;               now input as a vector, with one value per wavelength element
;    polyfwhm - keyword to take FWHM as a polynomial f'n of wavelength
...
if (keyword_set(polyfwhm) eq 0) then polyfwhm=0 else polyfwhm=1
...
if (fwhmflag eq 0) then sigma=newdel/2.354 else begin
  if (polyfwhm eq 0) then sigma = fwhm/2.354 else sigma=poly(newlam,fwhm)/2.354
endelse
